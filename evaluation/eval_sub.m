...
for i = skipsize+(index-1)*step+1:skipsize+index*step
    ...
    data_allmi = zeros(ori_rows,ori_cols,fea_cha,'single');
    img_mir = flipdim(img,2);
    ...
    for j = 1:size(scale_array,2)
        ...
        img_scalemi = imresize(img_mir,[new_rows new_cols],'bilinear');
        data_allmi = data_allmi + scale_process(net,img_scalemi,fea_cha,crop_size,ori_rows,ori_cols,mean_r,mean_g,mean_b);
    end
    
    data_allmi = data_allmi/size(scale_array,2);
    data_allmi = flipdim(data_allmi,2);
    data = (data_all + data_allmi)/2; %already exp process
