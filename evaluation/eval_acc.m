...
pathAnno = '/media/jason/9e141bf2-0912-419c-97d5-06acf3405238/jason/Dataset/cityscapes/gtFine_trainvaltest/gtFine/val/';
...
for i = 1: numel(filesPred)
    ...
    strPred = strPred{end};
    %strPred = strsplit(strPred,'.');
    strPredo = strPred ;
    strPred = str{2};
    %strPred
    if(strcmp(data_name,'cityscapes'))
        strPred = strrep(strPred,'leftImg8bit','gtFine_labelTrainIds');
    end
    %strPredsplit = strsplit( strPred,'_');
    
    %strPred
    %trAnno = [strPred{1}  '.png'];
    filePred = fullfile(pathPred, strPredo);
    fileAnno = fullfile(strPred);
